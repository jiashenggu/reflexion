...
def executor_factory(lang: str, is_leet: bool = False) -> Executor:
    if lang == "py" or lang == "python":
        if is_leet:
            ...
            from .leetcode_env.leetcode_env.utils import (
                PySubmissionFormatter,
                RsSubmissionFormatter,
            )

            return LeetExecutor(
                ProgrammingLanguage.PYTHON3, PyExecutor(), PySubmissionFormatter
            )
        else:
            return PyExecutor()
        ...
        if is_leet:
            ...
            return LeetExecutor(
                ProgrammingLanguage.RUST, RsExecutor(), RsSubmissionFormatter
            )
        else:
            return RsExecutor()
    else:
